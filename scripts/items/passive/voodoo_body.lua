...
local function getDamageScale(player)
    local damage_scale = Config.DAMAGE_SCALE
    if player:HasCollectible(CollectibleType.COLLECTIBLE_VOODOO_HEAD) then damage_scale = damage_scale + Config.VOODOO_HEAD_ADD end
...
ARAOI.Mod:AddCallback(ModCallbacks.MC_POST_ENTITY_TAKE_DMG, function (_, entity, damage, _, source)
