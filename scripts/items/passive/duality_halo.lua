...
local Devil_Angel_Room_Chance = {0, 0}
...
function modded_item:init(Mod)
    ...
    local function spawnConsolation()
        ...
        for i, v in ipairs(Devil_Angel_Room_Chance) do
            ...
            if v > 0 and chance <= v then
                ...
                local item = helper.item.SpawnCollectibleFromPool(item_pool, Isaac.GetCollectibleSpawnPosition(position), Vector.Zero, nil, true,
                ...
                if helper.player.AnyPlayerIs(PlayerType.PLAYER_KEEPER, PlayerType.PLAYER_KEEPER_B) then
                    item:MakeShopItem(-1)
                    item.Price = 1
                end
    ...
    local function bossDefeated()
        ...
        if room:IsCurrentRoomLastBoss() then
            ...
            Devil_Angel_Room_Chance = helper.misc.getDevilAngelRoomChance()
