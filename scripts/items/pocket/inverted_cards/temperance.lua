...
local DAMAGE_GIVEN = 0.75 -- *Default: `0.75` — Amount of damage that will be given per half a heart drained.*
...
function card:init(Mod)
    ...
    Mod:AddCallback(ModCallbacks.MC_EVALUATE_CACHE, function (_, player, flag)
        if flag == CacheFlag.CACHE_DAMAGE then
            ...
            player.Damage = player.Damage + (DAMAGE_GIVEN * hearts_lost) * helper.player.GetAproxDamageMultiplier(player)
        end
    end)
    ...
    if EID then
        EID:addCard(card.ID,
            ...
            "#{{ArrowUp}} +"..DAMAGE_GIVEN.." Damage for every {{HalfHeart}} Half a Heart lost"
        )
        EID:addTarotClothMetadata(card.ID, "Effect calculated as if Isaac had double his current health!")
    end
end
