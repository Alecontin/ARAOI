...
local function SetBlockTime(player, time)
    ARAOI.SaveDataManager:Key(block_time, ARAOI.PlayerUtils.GetId(player), 0, time == nil and MAX_BLOCK_TIME or time)
end
---@param player EntityPlayer
local function GetBlockTime(player)
    return ARAOI.SaveDataManager:Key(block_time, ARAOI.PlayerUtils.GetId(player), 0)
end
---@param player EntityPlayer
local function StopBlocking(player)
    ARAOI.SaveDataManager:Key(block_time, ARAOI.PlayerUtils.GetId(player), 0, 0)
end
...
ARAOI.Mod:AddCallback(ModCallbacks.MC_PRE_PLAYER_TAKE_DMG, function (_, player, _, damageFlags, source, _)
    ...
    -- We got hit outside our parry window, but we still blocked
        ...
        if data.VarData <= 0 then
            ...
            -- Spawn an attack
            local woosh = ARAOI.PlayerUtils.FireMelee(player, 1.5, -(player.Position - PARRIED_ENTITY.Position):Normalized(), false)
